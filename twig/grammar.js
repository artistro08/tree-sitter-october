 ...
 *
 * gbprod/tree-sitter-twig with two changes for October CMS:
 *
 *  1. Renamed from `twig` to `october_twig`. Zed registers grammars in one
 *     global namespace, so sharing the name `twig` with a standalone Twig
 *     extension means whichever loads last wins.
 *  2. `tag_statement` accepts named parameters. October tags take them -
 *     `{% partial 'footer' year=2026 %}`, `{% component 'x' k=1 %}` - and
 *     upstream Twig does not, so the `=` parsed as an error.
 */
